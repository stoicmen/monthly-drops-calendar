...
export async function buildMonthCalendar(
  ...
  // 3. 2nd Saturday: Surprise Character Drop (if not already scheduled)
  const secondSat = getNthWeekdayOfMonth(year, month, 6, 2);
  const secondSatStr = formatDate(secondSat);
  if (!scheduledDates.has(secondSatStr)) {
    events.push({
      date: secondSatStr,
      type: "surprise_drop",
      title: "Surprise Character Drop",
      tagline: "Unannounced. No teaser. Just drops.",
      thumbnailUrl: null,
      courseId: null,
      courseUrl: null,
      visibility: null,
